	...
	$.fn.zyPopup = function (options, param) {
		...
		return this.each(function () {
			...
			this.openPopup = function () {
				...
				function render() {
					var image = $(this), borderWidth = parseInt(zoomContent.css("borderLeftWidth")), maxImageWidth = windowWidth - (borderWidth * 2), maxImageHeight = windowHeight - (borderWidth * 2), imageWidth = image.width(), imageHeight = image.height();
			...
			this.createTailorPlug = function () {
				...
				var api = $.Jcrop("#tailorImg", {setSelect: [x1, y1, x2, y2], onChange: setCoords, onSelect: setCoords});
			...
			this.createCorePlug = function () {
				var params = {
					...
					onSelect: function (selectFiles, allFiles) {
						...
						window.funBindDelEvent = function () {
